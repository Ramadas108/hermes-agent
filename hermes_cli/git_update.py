...
# Stack-aware preparation path. This supersedes the historical single-commit
# implementation without changing production files or services.
def safe_update_stack(repo: Path, *, remote: str | None = None,
                       worktree: Path | None = None, run_tests: bool = True) -> int:
    repo = Path(repo).resolve()
    s = status(repo, fetch=True, remote=remote)
    print(f"Status: {status_text(s)}")
    if s.detached:
        ...
        return 2
    if s.ahead == 0:
        print("No local deployment commits to reapply; use the normal fast-forward path.")
        return 0
    deployment = run(repo, "rev-parse", "HEAD")
    stack = run(repo, "rev-list", "--reverse", f"{s.target}..{deployment}").splitlines()
    if not stack:
        print("No local deployment commits to reapply.")
        return 0
    print("LOCAL_STACK=" + ",".join(stack))
    root = worktree or Path(tempfile.mkdtemp(prefix="hermes-safe-update-", dir="/tmp"))
    created = worktree is None
    try:
        if root.exists() and any(root.iterdir()):
        ...
        print(f"Disposable worktree: {root}")
        for commit in stack:
            p = subprocess.run(["git", "-C", str(root), "cherry-pick", "--no-commit", commit],
                               text=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
            if p.returncode:
                print(f"CONFLICT while replaying {commit}.")
                print(p.stdout.strip())
                subprocess.run(["git", "-C", str(root), "cherry-pick", "--abort"], check=False,
